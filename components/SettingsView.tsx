...
const redactIfSensitive = (name: string, value: string): { displayValue: string; isRedacted: boolean } => {
    if (!value) {
        return { displayValue: '', isRedacted: false };
    }
    const lowerCaseName = name.toLowerCase();
    const sensitiveKeywords = ['key', 'secret', 'token'];
    
    // Check if any keyword is part of the variable name
    const isSensitive = sensitiveKeywords.some(keyword => lowerCaseName.includes(keyword));
    
    if (isSensitive) {
        return { displayValue: '••••••••••••••••', isRedacted: true };
    }
    
    return { displayValue: value, isRedacted: false };
};
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg">
                <div className="overflow-x-auto">
                    <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        ...
                        <tbody>
                            {envVars.map((v) => {
                                const { displayValue, isRedacted } = redactIfSensitive(v.name, v.value);
                                return (
                                <tr key={v.name} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 align-top">
                                    ...
                                    <td className="px-6 py-4">
                                        {v.value ? (
                                            <code className={`px-2 py-1 text-xs font-semibold rounded-full ${isRedacted ? 'text-gray-800 bg-gray-200 dark:bg-gray-600 dark:text-gray-300' : 'text-indigo-800 bg-indigo-100 dark:bg-indigo-900 dark:text-indigo-200'}`}>
                                                {displayValue}
                                            </code>
                                        ) : (
